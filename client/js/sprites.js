...
export async function loadSprites() {
  try {
      ...
      for (const density of ["low", "medium", "high"]) {
        for (
          ...
          const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
            .then((texture) => {
              state.residentialSprites[density].push({
                texture,
                ...spriteConfig,
                _jsonIndex: jsonIndex,
              });
              console.log(
                `[Sprites] Loaded residential.${density}: ${spriteConfig.id}`,
              );
            })
            .catch((err) => {
              console.warn(
                `[Sprites] Failed to load residential.${density} ${spriteConfig.id}:`,
                err,
              );
            });
          loadPromises.push(promise);
    ...
    if (spritesConfig.offices) {
      for (const density of ["low", "medium", "high"]) {
        for (
          ...
          const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
            .then((texture) => {
              state.officeSprites[density].push({
                texture,
                ...spriteConfig,
                _jsonIndex: jsonIndex,
              });
              console.log(
                `[Sprites] Loaded offices.${density}: ${spriteConfig.id}`,
              );
            })
            .catch((err) => {
              console.warn(
                `[Sprites] Failed to load offices.${density} ${spriteConfig.id}:`,
                err,
              );
            });
          loadPromises.push(promise);
    ...
    for (const serviceType of ["police", "hospital", "firestation"]) {
      if (spritesConfig[serviceType]) {
        for (let i = 0; i < spritesConfig[serviceType].length; i++) {
          ...
          const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
            .then((texture) => {
              state.serviceSprites[serviceType].push({
                texture,
                ...spriteConfig,
                _jsonIndex: jsonIndex,
              });
              console.log(
                `[Sprites] Loaded ${serviceType}: ${spriteConfig.id}`,
              );
            })
            .catch((err) => {
              console.warn(
                `[Sprites] Failed to load ${serviceType} ${spriteConfig.id}:`,
                err,
              );
            });
          loadPromises.push(promise);
    ...
    if (spritesConfig.park) {
      for (let i = 0; i < spritesConfig.park.length; i++) {
        ...
        const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
          ...
          .catch((err) => {
            console.warn(
              `[Sprites] Failed to load park ${spriteConfig.id}:`,
              err,
            );
          });
        loadPromises.push(promise);
    ...
    if (spritesConfig.suburban) {
      for (let i = 0; i < spritesConfig.suburban.length; i++) {
        ...
        const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
          ...
          .catch((err) => {
            console.warn(
              `[Sprites] Failed to load suburban ${spriteConfig.id}:`,
              err,
            );
          });
        loadPromises.push(promise);
    ...
    if (spritesConfig.industrial) {
      for (let i = 0; i < spritesConfig.industrial.length; i++) {
        ...
        const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
          ...
          .catch((err) => {
            console.warn(
              `[Sprites] Failed to load industrial ${spriteConfig.id}:`,
              err,
            );
          });
        loadPromises.push(promise);
    ...
    if (spritesConfig.crane) {
      for (let i = 0; i < spritesConfig.crane.length; i++) {
        ...
        const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
          ...
          .catch((err) => {
            console.warn(
              `[Sprites] Failed to load crane ${spriteConfig.id}:`,
              err,
            );
          });
        loadPromises.push(promise);
    ...
    if (spritesConfig.power_plant) {
      for (let i = 0; i < spritesConfig.power_plant.length; i++) {
        ...
        const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
          ...
          .catch((err) => {
            console.warn(
              `[Sprites] Failed to load power_plant ${spriteConfig.id}:`,
              err,
            );
          });
        loadPromises.push(promise);
    ...
    if (spritesConfig.water_tank) {
      for (let i = 0; i < spritesConfig.water_tank.length; i++) {
        ...
        const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
          ...
          .catch((err) => {
            console.warn(
              `[Sprites] Failed to load water_tank ${spriteConfig.id}:`,
              err,
            );
          });
        loadPromises.push(promise);
    ...
    if (spritesConfig.university) {
      for (let i = 0; i < spritesConfig.university.length; i++) {
        ...
        const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
          ...
          .catch((err) => {
            console.warn(
              `[Sprites] Failed to load university ${spriteConfig.id}:`,
              err,
            );
          });
        loadPromises.push(promise);
    ...
    if (spritesConfig.stadium) {
      for (let i = 0; i < spritesConfig.stadium.length; i++) {
        ...
        const promise = PIXI.Assets.load(`/sprites/${spriteConfig.file}`)
          ...
          .catch((err) => {
            console.warn(
              `[Sprites] Failed to load stadium ${spriteConfig.id}:`,
              err,
            );
          });
        loadPromises.push(promise);
    ...
    // Load grass tile textures
    for (let i = 1; i <= 5; i++) {
      const file = `sliced/grass_0${i}.png`;
      const promise = PIXI.Assets.load(`/sprites/${file}`)
        .then((texture) => {
          state.grassTextures.push(texture);
          console.log(`[Sprites] Loaded grass: grass_0${i}`);
        })
        .catch((err) => {
          console.warn(`[Sprites] Failed to load grass_0${i}:`, err);
        });
      loadPromises.push(promise);
    }
    ...
    const svcCount =
      ...
      state.serviceSprites.firestation.length;
    console.log(
...
export function generateStackedHouse(x, y, seed) {
  ...
  const bottomIndex = Math.floor(
    rng() * Math.max(1, state.houseBottoms.length),
  );
...
export function generateStackedOffice(x, y, seed, specifiedFloors = null) {
  ...
  const bottomIndex = Math.floor(
    rng() * Math.max(1, state.officeBottoms.length),
  );
