...
export function spawnPedestrian() {
  const {
    ...
    buildings,
    parcels,
    pedestriansContainer,
  } = state;

  if (
    animatedPedestrians.length >= state.MAX_PEDESTRIANS ||
    roads.length === 0
  ) {
    return;
  }
  ...
  const commercialBuildings = buildings.filter((b) =>
    ["shop", "office", "factory", "plaza"].includes(b.type),
  );
  ...
  if (commercialBuildings.length > 0 && Math.random() < 0.7) {
    targetBuilding =
      commercialBuildings[
        Math.floor(Math.random() * commercialBuildings.length)
      ];
  } else if (residentialBuildings.length > 0) {
    targetBuilding =
      residentialBuildings[
        Math.floor(Math.random() * residentialBuildings.length)
      ];
  }
  ...
  const validDirs = getValidDirections(
    ...
    Math.floor(pedestrian.y),
  );
  if (validDirs.length > 0) {
    pedestrian.dir = validDirs[Math.floor(Math.random() * validDirs.length)];
    pedestrian.targetX =
      Math.floor(pedestrian.x) +
      DIR_VECTORS[pedestrian.dir].dx +
      pedestrian.edgeOffset;
    pedestrian.targetY =
      Math.floor(pedestrian.y) +
      DIR_VECTORS[pedestrian.dir].dy +
      pedestrian.edgeOffset;
  }
  ...
  sprite.x = iso.x;
  sprite.y = iso.y;
  sprite.zIndex = Math.floor(pedestrian.y) * GRID_SIZE + Math.floor(pedestrian.x) + 0.5;
...
export function animatePedestrians(delta) {
  ...
  for (let i = animatedPedestrians.length - 1; i >= 0; i--) {
    ...
    if (ped.lifetime > ped.maxLifetime) {
      state.sceneLayer.removeChild(ped.sprite);
    ...
    ped.sprite.x = iso.x;
    ped.sprite.y = iso.y;
    ped.sprite.zIndex = Math.floor(ped.y) * GRID_SIZE + Math.floor(ped.x) + 1;
